...
Java_com_hybridcore_maliopt_MaliOptNative_detectExtensions(JNIEnv *env, jobject thiz) {
    ...
    /* Detecta contexto (GL4ES / ANGLE / NATIVE) */
    detect_context();
    LOGI("Contexto: %s", g_context);

    /* Coleta strings de info da GPU */
    ...
    if (glexts) {
        strncpy(g_raw_gl_ext, glexts, sizeof(g_raw_gl_ext) - 1);
    }

    /* Coleta extensoes EGL */
    EGLDisplay dpy = eglGetCurrentDisplay();
    if (dpy != EGL_NO_DISPLAY) {
        const char* eglexts = eglQueryString(dpy, EGL_EXTENSIONS);
        if (eglexts) {
            strncpy(g_raw_egl_ext, eglexts, sizeof(g_raw_egl_ext) - 1);
        }
    }
    ...
    /* ---- RESUMO NO LOGCAT ---- */
    LOGI("--- Fast Paths: FB_FETCH=%d | FB_FETCH_DS=%d | PLS=%d | MALI_BIN=%d",
         g_caps.FB_FETCH, g_caps.FB_FETCH_DEPTH_STENCIL, g_caps.PLS, g_caps.MALI_BINARY);
    LOGI("--- Textura:    ASTC_LDR=%d | ASTC_HDR=%d | ETC2=%d | ANISO=%d",
         g_caps.ASTC_LDR, g_caps.ASTC_HDR, g_caps.ETC2, g_caps.ANISOTROPIC);
    LOGI("--- Renderizacao: MSAA_RT=%d | SRGB=%d | PROG_BIN=%d | TIMER=%d",
         g_caps.MULTISAMPLED_RT, g_caps.SRGB, g_caps.PROGRAM_BINARY, g_caps.TIMER_QUERY);
    LOGI("--- Limites: MAX_TEX=%d | MAX_SAMPLES=%d | MAX_ANISO=%d | MAX_VA=%d",
...
Java_com_hybridcore_maliopt_MaliOptNative_getExtensionReport(JNIEnv *env, jobject thiz) {
    ...
    snprintf(report, sizeof(report),
        ...
        g_caps.GLES_VERSION,
        g_caps.FB_FETCH,
        g_caps.FB_FETCH_DEPTH_STENCIL,
        g_caps.PLS,
        g_caps.MALI_BINARY,
        g_caps.ASTC_LDR,
        g_caps.ASTC_HDR,
        g_caps.ETC2,
        g_caps.STANDARD_DERIVATIVES,
        g_caps.TEXTURE_FLOAT,
        g_caps.TEXTURE_FLOAT_LINEAR,
        g_caps.TEXTURE_HALF_FLOAT,
        g_caps.TEXTURE_HALF_FLOAT_LINEAR,
        g_caps.MULTISAMPLED_RT,
        g_caps.MULTISAMPLED_RT2,
        g_caps.SRGB,
        g_caps.SRGB_WRITE_CONTROL,
        g_caps.ANISOTROPIC,
        g_caps.PROGRAM_BINARY,
        g_caps.TEXTURE_STORAGE,
        g_caps.TEXTURE_NPOT,
        g_caps.MAP_BUFFER,
        g_caps.BUFFER_STORAGE,
        g_caps.TIMER_QUERY,
        g_caps.DEBUG_MARKER,
        g_caps.KHR_DEBUG,
        g_caps.ROBUSTNESS,
        ...
        g_caps.MAX_SAMPLES,
        g_caps.MAX_ANISOTROPY,
        g_caps.MAX_VERTEX_ATTRIBS,
        g_caps.MAX_COMBINED_TIU
    );
...
/* ============================================================
   GETTERS INDIVIDUAIS — FAST PATHS
   ============================================================ */
...
/* ============================================================
   GETTERS INDIVIDUAIS — COMPRESSAO
   ============================================================ */
JNIEXPORT jboolean JNICALL
...
/* ============================================================
   GETTERS INDIVIDUAIS — RENDERIZACAO
   ============================================================ */
JNIEXPORT jboolean JNICALL
